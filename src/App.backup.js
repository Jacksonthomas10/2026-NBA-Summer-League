...
function App() {
  ...
  const benchmarks = useMemo(
    () => ({
      medianImpactPerGame: getMedian(
        playerData.map(
          (player) => player.ImpactPerGame
        )
      ),
      ...
      */
      ...
      return {
        ...
        IsHiddenGem:
          ...
          player.ImpactPerGame >=
            benchmarks.medianImpactPerGame &&
          lowOrModerateUsage &&
          hiddenGemScore > 5,
      };
    });
  ...
  const teamFitPlayers = useMemo(() => {
    ...
    const dataset = {
      ImpactPerGame: analyzedPlayers.map(
        (player) => player.ImpactPerGame
      ),
  ...
  const displayedData = useMemo(() => {
    ...
    if (viewMode === "teamFit") {
      data = [...teamFitPlayers].slice(0, 30);
    } else if (viewMode === "hidden") {
      data = [...analyzedPlayers]
        .filter(
          (player) =>
            player &&
            Number.isFinite(Number(player.HiddenGemScore)) &&
            player.GP >= 1 &&
            player.MinPerGame >= 10
        )
        .sort(
          (a, b) =>
            Number(b.HiddenGemScore) - Number(a.HiddenGemScore)
        )
        .slice(0, 5);
    } else {
      data = [...analyzedPlayers];
    ...
    const filtered = data.filter((player) => {
      ...
      return (
        ...
        archetype.includes(normalizedSearch)
      );
    });

    /*
      Hidden Gems are already ranked before reaching this point.
      Do not apply the table's previous sort setting over the Top 5.
    */
    if (viewMode === "hidden") {
      return filtered;
    }
